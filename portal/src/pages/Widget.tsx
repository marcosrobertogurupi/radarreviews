...
import { Copy, RefreshCw, Eye, Code, Palette } from 'lucide-react'
...
export default function Widget({ tenantId }: Props) {
  ...
  async function rotateToken() {
    ...
    await supabase.from('tenants').update({ widget_token: newToken }).eq('id', tenantId)
    await loadTenant()
  }
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        ...
        <p className="page-subtitle">Configure o selo de reputação para o seu site</p>
      </div>

      {loading && !tenant ? (
        <div className="skeleton" style={{ height: 300, background: 'var(--bg-darker)', borderRadius: 12 }} />
      ) : (
        <div className="grid-2">
          {/* Configuração */}
          <div className="card" style={{ padding: 24 }}>
            <div className="section-title" style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20, fontSize: 16, fontWeight: 700 }}>
              <Palette size={18} color="var(--accent)" /> Estilo & Configuração
            </div>
            
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontSize: 12, color: 'var(--text-muted)', marginBottom: 6 }}>Tema Visual</label>
              <div style={{ display: 'flex', gap: 10 }}>
                <button className="btn active" style={{ flex: 1, background: 'var(--accent)', color: 'white', border: 'none', borderRadius: 8, padding: '8px 12px' }}>Claro</button>
                <button className="btn" style={{ flex: 1, opacity: 0.5, background: 'var(--bg-darker)', color: 'var(--text-muted)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px' }}>Escuro (Breve)</button>
              </div>
            </div>
            ...
            <div style={{ padding: 16, background: 'rgba(99,102,241,0.05)', borderRadius: 10, border: '1px solid var(--border)' }}>
              ...
              <code style={{ fontSize: 11, color: 'var(--text-secondary)', wordBreak: 'break-all', fontFamily: 'monospace' }}>
                {tenant?.widget_token || '—'}
              </code>
            </div>
          </div>
          ...
          <div className="card" style={{ padding: 24 }}>
            <div className="section-title" style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20, fontSize: 16, fontWeight: 700 }}>
              <Code size={18} color="var(--accent)" /> Código de Incorporação
            </div>
            <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16 }}>
              Copie e cole este código no final da tag <code>&lt;body&gt;</code> do seu site para exibir o widget.
            </p>
            ...
            <div style={{ marginTop: 24, padding: 16, border: '1px dashed var(--border)', borderRadius: 10, textAlign: 'center' }}>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                <Eye size={14} /> Pré-visualização Indisponível
              </div>
